...
const LOADING_STATUSES: ETradePhaseStatus[] = [
...
export const TradeButton = memo(function TradeButton({
  phase,
  direction,
  tokenSymbol,
  disabled,
  ...
  const isLoading = LOADING_STATUSES.includes(phase.status);
  const isSuccess = phase.status === ETradePhaseStatus.SUCCESS;
  const label = getPhaseLabel(phase.status, direction, tokenSymbol);
